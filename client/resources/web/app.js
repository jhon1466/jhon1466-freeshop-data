...
async function api(path, options = {}) {
  ...
  if (resp.status === 401) {
    const pin = prompt("PIN (set on the console in Settings → FTP):");
    if (pin) {
      localStorage.setItem("freeshopPin", pin);
...
async function loadFiles(path) {
  ...
  try {
    ...
    const body = await resp.json().catch(() => ({}));
    if (!resp.ok) {
      toast(body.error || "could not open that folder", true);
      return;
    }
    renderFiles(body);
  } catch {
...
function renderFiles(body) {
  ...
  if (body.parent !== undefined) {
    ...
    up.innerHTML = `<span class="files-icon">📁</span><span class="files-name">..</span>`;
  ...
  for (const entry of entries) {
    ...
    row.innerHTML = `<span class="files-icon">${entry.directory ? "📁" : "📄"}</span>` +
...
async function downloadFile(entry) {
  ...
  try {
    ...
    if (!resp.ok) {
      const body = await resp.json().catch(() => ({}));
      toast(body.error || "download failed", true);
      return;
    }
