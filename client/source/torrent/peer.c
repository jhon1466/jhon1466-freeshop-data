...
// src/core/peer.c, GPL-3.0) - see ../../THIRD_PARTY_NOTICES.md.
// Trimmed to the TCP-only transport - see peer.h's doc comment.
#include "peer.h"
#include "bencode.h"
...
// --- buffered send ---
// Whatever the socket does not accept immediately is queued in p->sbuf and
// drained by peer_flush() on POLLOUT. MSE encryption is layered above this
// by peer_send_raw.
static int peer_send_plain(peer_t *p, const uint8_t *data, uint32_t len) {
    ...
    if ((size_t)p->sbuf_len + len > sizeof(p->sbuf)) {
        torrent_debug_log("[peer] send queue overflow (%u+%u)", p->sbuf_len, len);
        return 0;
    }
...
// handshake bytes themselves go out before mse_active is set, so they are
// sent in the clear (the request's encrypted region is already ciphertext).
...
static int send_msg(peer_t *p, const uint8_t *payload, uint32_t plen) {
    ...
    if ((size_t)p->sbuf_len + 4 + plen > sizeof(p->sbuf)) {
        if (!peer_flush(p) ||
            ...
            torrent_debug_log("[peer] send queue full, dropping frame (%u+%u)",
                    p->sbuf_len, 4 + plen);
            return 0;
        }
    }
...
static int process_handshake(peer_t *p, const peer_ctx_t *ctx) {
    ...
    if (hs[0] != 19 || memcmp(hs+1, "BitTorrent protocol", 19) != 0) {
        torrent_debug_log("[peer] bad handshake");
        return -1;
    }
    if (memcmp(hs+28, ctx->info_hash, 20) != 0) {
        torrent_debug_log("[peer] info_hash mismatch");
        return -1;
    }
    ...
    torrent_debug_log("[peer] handshake ok ext=%d", p->supports_ext);
...
static int process_message(peer_t *p, const peer_ctx_t *ctx,
    ...
    if (msg_len > 1<<18) { // 256KB max message
        torrent_debug_log("[peer] oversized message %u", msg_len);
        return -1;
    }
    ...
    case MSG_UNCHOKE:
        if (p->am_choked)
            torrent_debug_log("[peer] unchoked");
        p->am_choked = 0;
        break;
    case MSG_INTERESTED:
        p->peer_interested = 1;
...
int peer_connected(peer_t *p, const peer_ctx_t *ctx) {
    if (p->mse_enabled) {
        ...
        // it rides inside the encrypted request.
...
int peer_process(peer_t *p, const peer_ctx_t *ctx,
    ...
    if (p->state == PS_MSE) {
        ...
        if (produced && !peer_send_raw(p, out, (uint32_t)produced))
            return -1;
        if (s == MSE_FAIL) {
            torrent_debug_log("[peer] mse handshake failed");
            return -1;
        }
        if (s == MSE_DONE) {
            p->mse_active = 1;
            ...
            p->state = PS_HANDSHAKE;
            torrent_debug_log("[peer] mse handshake ok");
        }
    }
...
int peer_rbuf_append(peer_t *p, const uint8_t *data, uint32_t len) {
    ...
    if (len > tail) {
        torrent_debug_log("[peer] recv buffer full");
        return -1;
    }
...
int peer_recv(peer_t *p, const peer_ctx_t *ctx,
    ...
    if (p->state == PS_CONNECTING) {
        ...
        if (err) {
            torrent_debug_log("[peer] connect failed: %s", strerror(err));
            return -1;
        }
