...
// src/core/bencode.c, GPL-3.0) - see ../../THIRD_PARTY_NOTICES.md.
#include "bencode.h"
#include <stdint.h>
#include <string.h>
...
// String: NNN:data
static int parse_str(const char **p, const char *end, const char **sval, size_t *slen) {
    const char *s = *p;
...
// Nesting cap - everything this ever actually parses is a handful of
// levels deep (root dict -> info -> files -> file dict -> path list is the
// deepest real case). Without a cap, a peer sending a few thousand bytes
// of nested 'l's in its extension handshake recurses once per byte and
// blows the thread's stack.
#define BE_MAX_DEPTH 16

// Skips one value without saving it.
...
int be_decode(const char **p, const char *end, be_node_t *out) {
    ...
    if (c == 'i') {
        ...
        out->type = BE_LIST;
        // Not recursed into here - the caller walks it with be_list_next.
        const char *tmp = *p;
...
int be_dict_next(const char **p, const char *end, const char **key, size_t *klen, be_node_t *val) {
...
int be_dict_get(const char *dict_start, const char *dict_end, const char *key, size_t klen, be_node_t *val) {
