...
// src/core/metainfo.c, GPL-3.0) - see ../../THIRD_PARTY_NOTICES.md.
#include "metainfo.h"
#include "bencode.h"
...
int metainfo_path_is_safe(const char *path) {
    ...
    if (strlen(path) >= MAX_NAME_LEN) return 0;
...
int metainfo_parse(const uint8_t *data, size_t len, metainfo_t *mi) {
    ...
    if (!be_decode(&p, end, &root) || root.type != BE_DICT) {
        torrent_debug_log("[meta] not a bencode dict");
        return 0;
    }

    // ---- info dict ----
    be_node_t info_node;
    if (!be_dict_get(root.buf, root.buf + root.raw_len, "info", 4, &info_node) || info_node.type != BE_DICT) {
        torrent_debug_log("[meta] no info dict");
        return 0;
    }

    // info_hash = SHA-1 of the raw bencoded info dict.
    ...
    if (!metainfo_path_is_safe(mi->name)) {
        torrent_debug_log("[meta] unsafe torrent name");
        return 0;
    }

    // piece length
    if (!be_dict_get(info_node.buf, info_node.buf + info_node.raw_len, "piece length", 12, &v) ||
        v.type != BE_INT) {
        torrent_debug_log("[meta] no piece length");
        return 0;
    }
    // Nothing downstream checks this: piece.c allocates piece_length bytes
    // and indexes at idx * piece_length. 256 MiB is far above any real
    // torrent (16 MiB is the practical ceiling) and still a sane allocation.
    if (v.ival <= 0 || v.ival > MAX_PIECE_LENGTH) {
        torrent_debug_log("[meta] bad piece length %lld", (long long)v.ival);
        return 0;
    }
    ...
    if (!be_dict_get(info_node.buf, info_node.buf + info_node.raw_len, "pieces", 6, &v) || v.type != BE_STR ||
        v.slen % 20 != 0) {
        torrent_debug_log("[meta] invalid pieces");
        return 0;
    }
    ...
    if (be_dict_get(info_node.buf, info_node.buf + info_node.raw_len, "files", 5, &files_node) &&
        ...
        if (fc > MAX_FILES) {
            torrent_debug_log("[meta] too many files (%u)", fc);
        ...
        while (fi < fc && be_list_next(&fp, fe, &item)) {
            ...
            if (be_dict_get(item.buf, item.buf + item.raw_len, "length", 6, &fv) && fv.type == BE_INT) {
                if (fv.ival < 0 || fv.ival > MAX_TOTAL_LENGTH) {
                    torrent_debug_log("[meta] bad file length %lld", (long long)fv.ival);
                    metainfo_free(mi);
                    return 0;
                }
                f->length = fv.ival;
            ...
            if (be_dict_get(item.buf, item.buf + item.raw_len, "path", 4, &path_node) &&
                ...
                while (be_list_next(&pp, pe, &part)) {
                    ...
                    // capping every component at MAX_NAME_LEN-1 individually
                    // let a few of them walk right off the end of the buffer.
            ...
            if (!metainfo_path_is_safe(f->path)) {
                torrent_debug_log("[meta] unsafe file path '%s'", f->path);
                metainfo_free(mi);
                return 0;
            }
            f->offset = offset;
        ...
        if (be_dict_get(info_node.buf, info_node.buf + info_node.raw_len, "length", 6, &lv) && lv.type == BE_INT) {
            if (lv.ival < 0 || lv.ival > MAX_TOTAL_LENGTH) {
                torrent_debug_log("[meta] bad file length %lld", (long long)lv.ival);
                metainfo_free(mi);
                return 0;
            }
    ...
    // every consumer of piece length/total length would inherit the mismatch.
    if (mi->num_pieces != (uint32_t)((mi->total_length + mi->piece_length - 1) / mi->piece_length)) {
        torrent_debug_log("[meta] %u pieces does not match %lld bytes at %lld", mi->num_pieces,
                           (long long)mi->total_length, (long long)mi->piece_length);
        metainfo_free(mi);
        return 0;
    }

    // Warn about the FAT32 4GB single-file limit.
    for (uint32_t i = 0; i < mi->num_files; i++) {
        if (mi->files[i].length > (int64_t)4 * 1024 * 1024 * 1024LL - 1)
            torrent_debug_log("[meta] WARNING: file '%s' exceeds 4 GB FAT32 limit", mi->files[i].path);
    }
    ...
    // ---- web seeds (BEP-19 url-list) ---- may be a single string or a list of strings.
    be_node_t ul;
    if (be_dict_get(root.buf, root.buf + root.raw_len, "url-list", 8, &ul)) {
        if (ul.type == BE_STR) {
            ...
            be_node_t url;
            while (mi->num_web_seeds < MAX_WEB_SEEDS && be_list_next(&up, ue, &url)) {
    ...
    torrent_debug_log("[meta] name='%s' hash=%s pieces=%u piece_len=%lld total=%lld files=%u trackers=%u "
                       "webseeds=%u",
                       mi->name, ih_hex, mi->num_pieces, (long long)mi->piece_length,
                       (long long)mi->total_length, mi->num_files, mi->num_trackers, mi->num_web_seeds);
    return 1;
}
...
int metainfo_load(const char *path, metainfo_t *mi) {
    ...
    if (!f) { torrent_debug_log("[meta] cannot open '%s'", path); return 0; }
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    rewind(f);
    if (sz <= 0 || sz > 10 * 1024 * 1024) {
        fclose(f);
        torrent_debug_log("[meta] file too large or empty");
        return 0;
    }
    ...
    fclose(f);
    int r = (got == (size_t)sz) && metainfo_parse(buf, got, mi);
    free(buf);
    return r;
}
