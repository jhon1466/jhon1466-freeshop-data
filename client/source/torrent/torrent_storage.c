...
// src/platform/storage.c, GPL-3.0) - see ../../THIRD_PARTY_NOTICES.md.
...
// Reopens a DISK file that storage_commit closed. The Switch's filesystem
// refuses to open a file for reading while another handle still has it open
// for writing, so a torrent install - which reads the container through its
// own handle while this side keeps downloading into it - needs the two to
// take turns instead of overlapping. storage_commit drops this side's
// handle; the next read or write here silently takes it back.
static int ensure_open(struct file_handle *fh) {
    if (fh->config.mode != STORAGE_FILE_DISK) return 0;
    if (fh->fp) return 1;
    fh->fp = fopen(fh->path, "r+b");
    if (!fh->fp) {
        torrent_debug_log("[storage] reopen failed: '%s'", fh->path);
        return 0;
    }
    return 1;
}
...
storage_t *storage_open_ex(const metainfo_t *mi, const char *outdir,
    ...
    for (uint32_t i = 0; i < mi->num_files; i++) {
        ...
        if (using_fallback &&
            ...
            torrent_debug_log("[storage] output path is too long, fallback failed");
            storage_close(s);
            return NULL;
        }
        ...
        if (!open_disk_file(fh)) {
            ...
            if (!fh->fp) {
                torrent_debug_log("[storage] cannot open '%s': %s",
        ...
        if (using_fallback)
            torrent_debug_log("[storage] long output path remapped to '%s'", fh->path);
    }
    return s;
}
...
int storage_flush(storage_t *s) {
    ...
    for (uint32_t i = 0; i < s->num_files; i++) {
        if (s->files[i].config.mode != STORAGE_FILE_DISK) continue;
        // A handle storage_commit already dropped needs nothing flushed -
        // closing it committed everything it held.
        if (!s->files[i].fp) continue;
        if (fflush(s->files[i].fp) != 0) return 0;
        // fflush only pushes stdio's own buffer into the filesystem driver.
        // That is enough when this handle is the only one that will ever
        // look at the file, but a torrent install now reads the container
        // through a *second*, concurrently-open handle while this one keeps
        // writing (see install_torrent.c) - so the driver has to actually
        // commit, or that reader can still see the pre-allocated zeros.
        // Best-effort: a devoptab without fsync support leaves the data
        // exactly as fflush left it rather than failing the write.
        fsync(fileno(s->files[i].fp));
    }
    return 1;
}
...
int storage_commit(storage_t *s) {
    ...
    for (uint32_t i = 0; i < s->num_files; i++) {
        struct file_handle *fh = &s->files[i];
        if (fh->config.mode != STORAGE_FILE_DISK || !fh->fp) continue;
        // Closed, NOT reopened: the handle has to stay gone so the caller's
        // own reader can open the file at all (see ensure_open). The next
        // write or read on this side reopens it lazily.
        fclose(fh->fp);
        fh->fp = NULL;
    }
    return 1;
...
int storage_file_path(storage_t *s, uint32_t index, char *out, size_t out_size) {
    if (!s || index >= s->num_files) return 0;
    struct file_handle *fh = &s->files[index];
    if (fh->config.mode != STORAGE_FILE_DISK) return 0;
    snprintf(out, out_size, "%s", fh->path);
    return 1;
}
