...
// src/core/dht.c, GPL-3.0) - see ../../THIRD_PARTY_NOTICES.md.
// Glue layer for jech's dht.c (MIT). Implements the user-provided
// callbacks and wraps the public API. See dht_engine.h for why this is the
// one file in the torrent engine allowed a real background thread.
#include "dht_engine.h"
#include "torrent_util.h"
#include "torrent_log.h"
// dht/dht.h needs stdio.h for FILE*
#include <stdio.h>
#include "dht/dht.h"
#include <poll.h>
#include <pthread.h>
...
int dht_sendto(int s, const void *buf, int len, int flags,
    ...
    if (r < 0) {
        ...
        if (now - last_log_ms >= 10000) {
            torrent_debug_log("[dht] sendto failed errno=%d suppressed=%u",
                    errno, suppressed);
            last_log_ms = now;
            suppressed = 0;
        } else {
...
static void dht_callback(void *closure, int event,
    ...
    if (event == DHT_EVENT_VALUES) {
        ...
        torrent_debug_log("[dht] search done");
    }
}
...
static void dht_bootstrap_ping(void) {
    ...
    for (size_t i = 0; i < BOOTSTRAP_COUNT; i++) {
        resolved[i] = net_resolve(BOOTSTRAP[i].host, BOOTSTRAP[i].port,
                                  &addrs[i]);
        if (resolved[i])
            torrent_debug_log("[dht] bootstrap -> %s:%u",
...
static void *dht_thread_main(void *arg) {
    ...
    while (!atomic_load(&g_dht.stop)) {
        ...
        if (rebootstrap) {
            torrent_debug_log("[dht] no good nodes, re-bootstrapping");
            dht_bootstrap_ping();
...
static int dht_engine_start(void) {
    ...
    if (g_dht.fd == INVALID_SOCK)
        return 0;
    if (dht_init(g_dht.fd, -1, node_id, (const uint8_t*)"FRSH1") < 0) {
        net_close(g_dht.fd);
        return 0;
    }
    ...
    if (cached_count > 0)
        torrent_debug_log("[dht] cache: pinged %d nodes", cached_count);
    ...
    torrent_debug_log("[dht] init port=%u", DHT_SHARED_PORT);
    return 1;
}
    ...
    if (g_cache_path[0]) {
        ...
        if (num > 0) {
            ...
            if (dht_cache_write(g_cache_path, g_dht.node_id, nodes, num))
                torrent_debug_log("[dht] saved %d nodes to %s", num, g_cache_path);
        }
    }
    dht_uninit();
    net_close(g_dht.fd);
...
dht_session_t *dht_attach(const uint8_t info_hash[20],
    ...
    torrent_debug_log("[dht] attach announce_port=%u sessions=%d",
...
void dht_detach(dht_session_t *s) {
    ...
    free(s);
    torrent_debug_log("[dht] detach sessions=%d", sessions);
}
