...
// src/core/piece.c, GPL-3.0) - see ../../THIRD_PARTY_NOTICES.md.
// Trimmed to inline hashing - see piece.h's doc comment.
#include "piece.h"
...
// Slides the cursor forward past completed pieces. Amortized O(1): each
// order position is walked over at most once per completion/rewind cycle.
...
static void mark_piece_done(piece_mgr_t *pm, uint32_t idx, uint32_t plen) {
    ...
    torrent_debug_log("[piece] verified piece %u/%u", pm->num_done, pm->num_pieces);
}
...
void piece_mgr_mark_pending(piece_mgr_t *pm, uint32_t idx) {
    ...
    piece_slot_t *sl = &pm->slots[idx];
    if (sl->state == PS_DONE)
        return; // complete - never re-alloc a buf
...
int piece_mgr_got_block(piece_mgr_t *pm, uint32_t idx, uint32_t offset,
    ...
    if (memcmp(digest, expected, 20) != 0) {
        torrent_debug_log("[piece] SHA1 MISMATCH piece %u - resetting", idx);
        reset_piece(pm, idx);
        return 0;
    }
    ...
    if (!storage_write(pm->store, abs_off, sl->buf, (size_t)plen)) {
        ...
        torrent_debug_log("[piece] write error piece %u: %s", idx,
...
// Shared scratch for the verification paths: they run one piece at a time,
// so a single lazily-allocated piece_length buffer replaces a malloc/free
// churn of one piece size per verified piece.
...
int piece_mgr_verify_piece(piece_mgr_t *pm, uint32_t idx) {
    ...
    if (storage_read(pm->store, abs_off, buf, (size_t)plen) != plen) {
        torrent_debug_log("[piece] final read error piece %u - resetting", idx);
        valid = 0;
    } else {
        sha1(buf, (size_t)plen, digest);
        if (memcmp(digest, pm->mi->piece_hashes + idx * 20, 20) != 0) {
            torrent_debug_log("[piece] final SHA1 MISMATCH piece %u - resetting", idx);
            valid = 0;
        }
    }
...
// Marks a piece DONE without hashing it. Shared by the skipped-range branch
...
int piece_mgr_check_existing(piece_mgr_t *pm, uint32_t idx) {
    ...
    if (pm->skip_disk_verify)
        return 0; // known-fresh download - not present, no I/O to prove it
...
void piece_mgr_set_priority(piece_mgr_t *pm, uint32_t first, uint32_t last) {
    if (!pm) return;
    if (first > last) {
        pm->priority_active = 0;
        return;
    }
    pm->priority_first = first;
    pm->priority_last = last;
    pm->priority_active = 1;
}

uint32_t piece_mgr_pick(const piece_mgr_t *pm,
                        const uint8_t *peer_bf, uint32_t bf_bytes) {
    // Whatever a consumer is blocked on comes first. Falling through when
    // this peer holds none of it keeps the rest of the swarm busy on the
    // normal order rather than stalling behind one narrow range.
    if (pm->priority_active) {
        uint32_t last = pm->priority_last < pm->num_pieces ? pm->priority_last
                                                            : pm->num_pieces - 1;
        uint32_t pending_candidate = (uint32_t)-1;
        for (uint32_t i = pm->priority_first; i <= last; i++) {
            if (pm->slots[i].state == PS_DONE) continue;
            if (!request_allowed(pm, i)) continue;
            if (!(i / 8 < bf_bytes && bf_has(peer_bf, i))) continue;
            if (pm->slots[i].state == PS_EMPTY) return i;
            if (pending_candidate == (uint32_t)-1 &&
                slot_has_requestable_block(&pm->slots[i]))
                pending_candidate = i;
        }
        if (pending_candidate != (uint32_t)-1) return pending_candidate;
    }
