...
// src/core/mse.c, GPL-3.0) - see ../../THIRD_PARTY_NOTICES.md.
#include "mse.h"
#include "torrent_sha1.h"
#include "torrent_util.h" // rand_bytes
#include <string.h>
...
void rc4_init(rc4_t *rc4, const uint8_t *key, size_t key_len) {
    for (int n = 0; n < 256; ++n) rc4->s[n] = (uint8_t)n;
    rc4->i = rc4->j = 0;
    if (!key_len) return;
    uint8_t j = 0;
    for (int n = 0; n < 256; ++n) {
...
static void dh_modexp(const uint8_t *base, size_t base_len, const uint8_t exp[MSE_DH_LEN],
...
void mse_dh_secret(const uint8_t priv[MSE_DH_LEN], const uint8_t peer_pub[MSE_DH_LEN], uint8_t secret[MSE_DH_LEN]) {
...
static void req_hash(const char tag[4], const uint8_t *data, size_t len, uint8_t out[20]) {
    sha1_ctx_t ctx;
    sha1_init(&ctx);
...
mse_status_t mse_client_start(mse_client_t *c, const uint8_t info_hash[20], const uint8_t priv[MSE_DH_LEN],
    ...
    *produced = 0;
    if (ia_len > MSE_MAX_IA || pad_len > MSE_MAX_PAD) return MSE_FAIL;
    memset(c, 0, sizeof(*c));
    ...
    if (out_cap < MSE_DH_LEN + pad_len) return MSE_FAIL;
    uint8_t pub[MSE_DH_LEN];
    mse_dh_public(priv, pub);
    memcpy(out, pub, MSE_DH_LEN);
    if (pad_len) memcpy(out + MSE_DH_LEN, pad, pad_len);
    *produced = MSE_DH_LEN + pad_len;
    c->state = ST_READ_PUB;
    return MSE_CONTINUE;
}

static mse_status_t emit_request(mse_client_t *c, const uint8_t secret[MSE_DH_LEN], uint8_t *out, size_t out_cap,
                                  size_t *produced) {
    ...
    size_t n = 0;
    memset(plain + n, 0, MSE_VC_LEN);
    n += MSE_VC_LEN; // VC = 8 zeros
    plain[n++] = 0;
    plain[n++] = 0;
    plain[n++] = 0;
    plain[n++] = MSE_CRYPTO_RC4; // crypto_provide
    plain[n++] = 0;
    plain[n++] = 0; // len(padC) = 0
    plain[n++] = (uint8_t)(c->ia_len >> 8);
    plain[n++] = (uint8_t)(c->ia_len & 0xFF); // len(IA)
    memcpy(plain + n, c->ia, c->ia_len);
    n += c->ia_len;

    size_t total = 20 + 20 + n;
    if (out_cap < total) return MSE_FAIL;
    memcpy(out, req1, 20);
...
mse_status_t mse_client_feed(mse_client_t *c, const uint8_t *in, size_t in_len, size_t *consumed, uint8_t *out,
    ...
    *produced = 0;
    ...
    if (c->state == ST_READ_PUB) {
        if (in_len < MSE_DH_LEN) return MSE_CONTINUE;
        ...
        *consumed += MSE_DH_LEN;
        ...
        // The peer's encrypted VC == keyB keystream[0..7] - precompute to scan for it.
    ...
    if (c->state == ST_SYNC_VC) {
        ...
        if (found) {
            ...
            *consumed += skip;
            ...
            if (c->skipped > MSE_MAX_PAD + MSE_DH_LEN) { c->state = ST_FAIL; return MSE_FAIL; }
            return MSE_CONTINUE;
        }
    }
    ...
    if (c->state == ST_READ_SELECT) {
        while (c->select_have < sizeof(c->select_buf) && *consumed < in_len) {
            rc4_crypt(&c->recv_rc4, in + *consumed, &c->select_buf[c->select_have], 1);
            ++c->select_have;
            ++*consumed;
        }
        if (c->select_have < sizeof(c->select_buf)) return MSE_CONTINUE;
        uint32_t select = ((uint32_t)c->select_buf[0] << 24) | ((uint32_t)c->select_buf[1] << 16) |
                           ((uint32_t)c->select_buf[2] << 8) | (uint32_t)c->select_buf[3];
        uint32_t padd = ((uint32_t)c->select_buf[4] << 8) | c->select_buf[5];
        if (!(select & MSE_CRYPTO_RC4) || padd > MSE_MAX_PAD) { c->state = ST_FAIL; return MSE_FAIL; }
    ...
    if (c->state == ST_READ_PADD) {
        ...
        if (c->padd_remaining > 0) return MSE_CONTINUE;
        c->state = ST_DONE;
        return MSE_DONE;
    }

    if (c->state == ST_DONE) return MSE_DONE;
