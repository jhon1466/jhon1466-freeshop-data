...
# client/VERSION is the single source of truth for the shipped version -
# scripts/publish-client-release.js cross-checks a release tag against it so
# a mismatched tag is caught here instead of consoles looping on an update
# that never appears to apply.
PIPENSX_VERSION="${PIPENSX_VERSION:-$(cat "${ROOT}/VERSION" 2>/dev/null || echo 0.0.0)}"
...
echo "Built: ${BUILD_DIR}/freeshop-client.nro"
