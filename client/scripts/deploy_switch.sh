...
MTP_DIR="${MTP_DIR:-}"
NRO_SRC="${NRO_SRC:-${ROOT}/build-switch/freeshop-client.nro}"
DEPLOY_CLEAN="${DEPLOY_CLEAN:-0}"

if [[ -z "$MTP_DIR" ]]; then
    echo "Set MTP_DIR to the target directory, for example:" >&2
    echo "  make deploy MTP_DIR='mtp://DEVICE/1: SD Card/switch/freeshop-client'" >&2
    exit 2
fi
...
if [[ "$DEPLOY_CLEAN" == "1" ]]; then
    ...
    echo "==> Replacing only freeshop-client.nro (set DEPLOY_CLEAN=1 for a clean deploy)"
    gio remove "$MTP_DIR/freeshop-client.nro" 2>/dev/null || true
fi

echo "==> Copying freeshop-client.nro"
gio copy "$NRO_SRC" "$MTP_DIR/freeshop-client.nro"
