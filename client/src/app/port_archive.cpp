...
// path ÔåÆ destination relative to /switch, or empty if no switch segment /
    ...
    while (pos < folded.size()) {
        ...
        if (folded.compare(pos, end - pos, "switch") == 0) {
            ...
            if (lowerAscii(top) == "pipensx" ||
                lowerAscii(top) == "freeshop-client")
                return {};
            return relative;
        }
        if (slash == std::string::npos)
...
bool writeBytes(const std::string& path, const uint8_t* data, size_t size,
    ...
    if (!parent.empty() && !mkdirs(parent)) {
        error = "No se pudieron crear los directorios de destino.";
        return false;
    }
    const std::string temporary = path + ".pipensx-part";
    std::FILE* out = std::fopen(temporary.c_str(), "wb");
    if (!out) {
        error = "No se pudo crear el archivo extraído.";
        return false;
    }
    size_t written = 0;
    while (written < size) {
        ...
        if (std::fwrite(data + written, 1, chunk, out) != chunk) {
            std::fclose(out);
            std::remove(temporary.c_str());
            error = "No se pudo escribir el archivo extraído.";
            return false;
        }
        written += chunk;
        if (progress)
    ...
    if (std::fflush(out) != 0 || std::fclose(out) != 0) {
        std::remove(temporary.c_str());
        error = "No se pudo vaciar el archivo extraído.";
        return false;
    }
    std::remove(path.c_str());
    if (std::rename(temporary.c_str(), path.c_str()) != 0) {
        std::remove(temporary.c_str());
        error = "No se pudo confirmar el archivo extraído.";
        return false;
    }
    return true;
}
    ...
    while (!cancelled.load(std::memory_order_relaxed)) {
        if (std::fread(local.data(), 1, 30, file) != 30) {
            if (std::feof(file))
                break;
            std::fclose(file);
            error = "Cabecera ZIP local truncada.";
            return false;
        }
        ...
        if (sig != 0x04034b50) {
            std::fclose(file);
            error = "Cabecera ZIP local no válida.";
            return false;
        }
        ...
        if ((nameLen &&
            ...
            std::fclose(file);
            error = "Cabecera de entrada ZIP truncada.";
            return false;
        }
        ...
        if (compSize > 0) {
            ...
            if (std::fread(compressed.data(), 1, compSize, file) != compSize) {
                std::fclose(file);
                error = "Datos de entrada ZIP truncados.";
                return false;
            }
        } else {
            compressed.clear();
        ...
        if (method == 0) {
            if (compSize != uncompSize) {
                std::fclose(file);
                error = "El tamaño ZIP almacenado no coincide.";
                return false;
            }
            ...
            if (inflateInit2(&stream, -MAX_WBITS) != Z_OK) {
                std::fclose(file);
                error = "No se pudo iniciar la descompresión ZIP.";
                return false;
            }
            ...
            if (zrc != Z_STREAM_END || stream.total_out != uncompSize) {
                std::fclose(file);
                error = "Falló la descompresión ZIP.";
                return false;
            }
            ...
            std::fclose(file);
            error = "Método de compresión ZIP no compatible.";
            return false;
        }
    }
    std::fclose(file);
...
bool streamFolderDecode(const CSzArEx& db, UInt32 folderIndex,
    ...
    if (SzGetNextFolderItem(&folder, &sd) != SZ_OK || sd.Size != 0) {
        error = "No se pudo analizar una carpeta 7z.";
        return false;
    }
    if (!folderStreamable(folder)) {
        error = "Compresión 7z no compatible para extracción en streaming.";
        return false;
    }
    ...
    if (LookInStream_SeekTo(inStream, db.dataPos + packOffset) != SZ_OK) {
        error = "No se pudo buscar dentro del archivo 7z.";
        return false;
    }
    ...
    if (hasFilter && filterId == kMethodDelta) {
        if (folder.Coders[1].PropsSize != 1) {
            error = "Filtro delta no compatible.";
            return false;
        }
        deltaDistance =
    ...
    auto emitAlignedFilter = [&](Byte* buf, size_t size) -> bool {
        ...
        if (filterId == kMethodArm64)
            ...
            error = "Filtro de rama 7z no compatible.";
            return false;
        }
    ...
    if (mainCoder.MethodID == kMethodCopy) {
        if (packSize != unpackTotal) {
            error = "El tamaño de la carpeta 7z copiada no coincide.";
            return false;
        }
    ...
    if (mainCoder.MethodID == kMethodLzma2) {
        if (mainCoder.PropsSize != 1) {
            error = "Propiedades LZMA2 no válidas.";
            return false;
        }
        CLzma2Dec state;
        ...
        if (Lzma2Dec_Allocate(&state, data[mainCoder.PropsOffset], alloc) !=
            SZ_OK) {
            error = "No se pudo asignar el decodificador LZMA2.";
            return false;
        }
        Lzma2Dec_Init(&state);
        ...
        while (ok && unpacked < unpackTotal) {
            ...
            const SRes dr = Lzma2Dec_DecodeToBuf(
                ...
                &status);
            if (dr != SZ_OK) {
                error = "Falló la decodificación LZMA2.";
                ok = false;
                break;
            }
            if (inProcessed > 0) {
            ...
            if (inProcessed == 0 && outProcessed == 0) {
                if (status == LZMA_STATUS_FINISHED_WITH_MARK ||
                    ...
                    break;
                error = "La decodificación LZMA2 se detuvo.";
                ok = false;
                break;
            }
        }
        ...
        if (unpacked != unpackTotal) {
            error = "El tamaño de salida LZMA2 no coincide.";
            return false;
        }
        if (!flushCarry() || !sink.finish()) {
            error = sink.error.empty() ? "Unable to finish extract."
    ...
    if (mainCoder.MethodID == kMethodLzma) {
        ...
        if (LzmaDec_Allocate(&state, data + mainCoder.PropsOffset,
            ...
            error = "No se pudo asignar el decodificador LZMA.";
            return false;
        }
        LzmaDec_Init(&state);
        std::vector<Byte> outBuf(kStreamOutChunkBytes);
        ...
        while (ok && unpacked < unpackTotal) {
            ...
            const SRes dr = LzmaDec_DecodeToBuf(
                ...
                &status);
            if (dr != SZ_OK) {
                error = "Falló la decodificación LZMA.";
                ok = false;
                break;
            }
            if (inProcessed > 0) {
            ...
            if (inProcessed == 0 && outProcessed == 0) {
                if (status == LZMA_STATUS_FINISHED_WITH_MARK ||
                    status == LZMA_STATUS_MAYBE_FINISHED_WITHOUT_MARK)
                    break;
                error = "La decodificación LZMA se detuvo.";
                ok = false;
                break;
            }
        }
        ...
        if (!ok)
            return false;
        if (unpacked != unpackTotal) {
            error = "El tamaño de salida LZMA no coincide.";
            return false;
        }
        if (!flushCarry() || !sink.finish()) {
            error = sink.error.empty() ? "Unable to finish extract."
    ...
    error = "Método 7z no compatible para extracción en streaming.";
    return false;
}
...
bool extract7zRam(CSzArEx& db, ILookInStreamPtr inStream,
    ...
    for (UInt32 i = 0; i < db.NumFiles; ++i) {
        ...
        if (res != SZ_OK) {
            error = res == SZ_ERROR_MEM
                ? "No hay suficiente RAM libre para extraer este bloque sólido 7z."
                : "No se pudo extraer una entrada 7z.";
            ok = false;
            break;
        }
    ...
    if (ok && extracted == 0) {
        error = "El archivo no tiene archivos switch/ para extraer.";
        return false;
    }
    return ok;
}
...
bool extract7z(const std::string& archivePath, const std::string& targetRoot,
    ...
    if (InFile_Open(&archiveStream.file, archivePath.c_str()) != 0) {
        error = "No se pudo abrir el archivo 7z.";
        return false;
    }
    ...
    if (!lookStream.buf) {
        ...
        error = "Sin memoria al abrir el archivo 7z.";
        return false;
    }
    ...
    if (res != SZ_OK) {
        ...
        File_Close(&archiveStream.file);
        error = "No se pudo analizar el archivo 7z.";
        return false;
    }
...
bool probe7z(const std::string& archivePath, PortArchiveProbe& out) {
    ...
    ISzAlloc allocTempImp = {SzAlloc, SzFree};
    if (InFile_Open(&archiveStream.file, archivePath.c_str()) != 0) {
        out.error = "No se pudo abrir el archivo 7z.";
        return false;
    }
    FileInStream_CreateVTable(&archiveStream);
    archiveStream.wres = 0;
    ...
    if (!lookStream.buf) {
        File_Close(&archiveStream.file);
        out.error = "Sin memoria al abrir el archivo 7z.";
        return false;
    }
    lookStream.bufSize = kLookBufBytes;
    lookStream.realStream = &archiveStream.vt;
    ...
    if (res != SZ_OK) {
        ...
        ISzAlloc_Free(&allocImp, lookStream.buf);
        File_Close(&archiveStream.file);
        out.error = "No se pudo analizar el archivo 7z.";
        return false;
    }
    for (UInt32 folder = 0; folder < db.db.NumFolders; ++folder) {
        const UInt64 unpack = SzAr_GetFolderUnpackSize(&db.db, folder);
...
bool probePortArchive(const std::string& archivePath, PortArchiveProbe& out) {
    ...
    out.error = "Tipo de archivo de port no compatible.";
    return false;
}
...
bool extractPortArchive(const std::string& archivePath,
    ...
    if (base == "switch.7z" || endsWithCi(archivePath, ".7z"))
        return extract7z(archivePath, targetRoot, cancelled, progress,
                         currentFile, error);
    error = "Tipo de archivo de port no compatible.";
    return false;
}
