...
BatchPreparation CatalogBatchInstaller::prepareViaDebrid(
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        ...
        if (!createDebridWithMetainfoFallback(
            ...
            result.failures_.push_back(
                {entry, err.empty() ? "El servicio debrid no pudo abrir este torrent. Prueba con otra versión." : err});
            continue;
        }
        Pending p;
        p.entry = entry;
    ...
    for (Pending& p : pending) {
        if (p.haveInfo && p.info.phase == DebridInfo::Phase::Failed) {
            std::string ignored;
            provider.remove(p.id, ignored);
            result.failures_.push_back({p.entry, "El servicio debrid no pudo abrir este torrent. Prueba con otra versión."});
            continue;
        }
        if (p.ready) {
