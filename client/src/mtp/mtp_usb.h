...
// USB transport for the MTP responder (mtp_ptp.hpp) - one bulk OUT endpoint
...
// the MTP vendor-extension signature in GetDeviceInfo (see mtp_ptp.cpp) is
...
// chunk of a large data phase - see mtp_ptp.cpp). `timeout_ns` bounds the
// wait so a caller polling this from a worker-thread loop (checking for
// cancel/stop every iteration) doesn't stall indefinitely with nothing
// connected. Returns the byte count actually transferred, 0 on timeout, or
// -1 on a real transport error (cable unplugged mid-transfer, endpoint
// stalled, etc.) - callers should treat -1 as "the session is over".
