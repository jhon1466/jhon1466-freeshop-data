...
void testMissingFileUsesSafeDefaults() {
    ...
    // "auto" keeps the console's system language, so a Spanish-language
    // Switch gets a Spanish UI on first launch with no user action.
...
void testUpdatePersistsEveryPublicSetting() {
    ...
    changed.realdebridApiKey = "RD1234567890abcdef";
    changed.catalogSourceUrl = "https://cdn.example.com/repo/switch_games.json";
...
void testVersionOneResetsActiveDownloads() {
    cleanup();
    {
        ...
        output << R"({"version":1,"max_active_downloads":4,)"
               << R"("language":"es","web_server_pin":"4242"})";
    }
    {
        AppSettings settings(SettingsPath, LegacyPath);
        std::string error;
        ...
        assert(settings.get().language == "es");
...
void testFutureVersionIsRejected() {
    ...
    {
        std::ofstream output(SettingsPath);
        output << R"({"version":99,"language":"es"})";
    }
    AppSettings settings(SettingsPath, LegacyPath);
    std::string error;
    assert(!settings.load(error));
